...
const MonsterCard: React.FC<MonsterCardProps> = ({
  ...
  const calculatePowerRating = (monster: Monster) => {
    return Math.round(
  ...
  const getRatingColor = (rating: number) => {
    ...
    if (rating >= 30) return "info";
    return "success";
  };

  const powerRating = calculatePowerRating(monster);
    ...
    >
      <Box sx={{ position: "relative", pt: { xs: 2.5, sm: 3 } }}>
        ...
        {onDelete && (
          ...
          >
            ...
            >
              <DeleteOutline sx={{ fontSize: { xs: "0.9rem", sm: "1rem" } }} />
            </IconButton>
          </Box>
        )}
      ...
      >
        ...
        >
          ...
          <Chip
            label={`⭐ ${powerRating}`}
            color={getRatingColor(powerRating)}
            size="small"
            sx={{
              fontWeight: "bold",
