...
const BattleArena: React.FC<BattleArenaProps> = React.memo(
  ({ monster1, monster2, onBattleComplete, onReset }) => {
    ...
    const [attackingMonster, setAttackingMonster] = useState<string | null>(
      null
    );
    ...
    }, [monster1.hp, monster2.hp]);

    const handleInstantBattle = useCallback(() => {
    ...
    }, [monster1, monster2, onBattleComplete]);

    const handleAnimatedBattle = useCallback(async () => {
      setIsAnimating(true);
      setCurrentRound(0);
      setMonster1CurrentHp(monster1.hp);
      setMonster2CurrentHp(monster2.hp);
    ...
    const handleReset = useCallback(() => {
      ...
      setCurrentRoundData(null);
      onReset?.();
    }, [monster1.hp, monster2.hp, onReset]);

    const getMonsterCurrentHp = useCallback(
      (monsterId: string) => {
        return monsterId === monster1.id
          ? monster1CurrentHp
          : monster2CurrentHp;
      },
      [monster1.id, monster1CurrentHp, monster2CurrentHp]
    );

    const isMonster1Winner = useMemo(
      () => battleResult?.winner.id === monster1.id,
      [battleResult?.winner.id, monster1.id]
    );

    const isMonster2Winner = useMemo(
      () => battleResult?.winner.id === monster2.id,
      [battleResult?.winner.id, monster2.id]
    );

    const renderBattleMonsterCard = useCallback(
      (
        monster: Monster,
        isWinner?: boolean,
        isAttacking?: boolean,
      ...
      [battleFinished, getMonsterCurrentHp, currentRoundData]
    );
